...
- **Source layout** (no `src/` — files live at the repo root):
  ...
  - `lib/` — server-side scanner + layout math. `scan.ts` walks the repo (every file, not just TS — see *File descriptions* below) and builds a recursive `DirectoryNode` tree with per-file line counts (sorted children descending by size — precondition for the treemap), a best-effort `description` from each file's leading comment, and a per-file import graph (`imports` + reverse-indexed `importedBy`, internal TS files only — npm packages and non-TS files have empty graphs). `treemap.ts` is a pure squarified-treemap function. `classify.ts` maps TS symbols to `SymbolKind`. `types.ts` is the shared shape (FileNode/DirectoryNode discriminated union, also exports `ActiveAgent`).
  ...
  - **What gets visualized.** The scanner's `IGNORE` list in `lib/scan.ts` is the source of truth — it skips `node_modules`, build output (`.next`, `dist`, `out`, `build`), `.git`, runtime state (`.constellation`, `.claude/worktrees`, `.claude/settings.local.json`), generated/binary files (`*.tsbuildinfo`, `.DS_Store`, common image/font/archive extensions), `next-env.d.ts`, and `package-lock.json` (tracked in git but excluded here because at thousands of lines it would dwarf real source). Everything else — TS, shell, markdown, JSON, CSS, configs, dotfiles like `.gitignore` and `.claude/settings.json` — becomes a tile. TS files get full export + import-graph rendering; non-TS files render as plain tiles (filename + line count + best-effort description, empty Exports/Imports).
